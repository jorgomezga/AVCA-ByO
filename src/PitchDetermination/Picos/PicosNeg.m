function [P, i_P, picos]=PicosNeg( vSignal, iFs, iInicio, iFinal, T )

% Obtains the amplitudes P and the positions i_P of the negative peaks, 
% cycle by cycle, of the input voice. It also returns the peak signal 
% containing the peaks found in the analysis section.
%
% Input parameters
%   vSignal:     is a column vector containing the complete speech signal
%   iFs:         is the sampling frequency
%   iInicio:     is the beginning sample
%   iFinal:      is the last sample
...
%
% Output parameters:
%   P:          amplitudes of negative peaks, cycle to cycle, of a speech
%               signal
...
% The variable tam_seg contains the durations in samples of the segments that have been used
% in the calculation of T (Each segment lasts 3 times the pitch period of the previous segment; the
...
% Negative peaks are searched for each segment. There should be 3 peaks
...
% k is an index to reference vectors P (amplitudes of negative peaks), and
      ...
      while mueBusq+T(n)<=IniSeg+tam_seg(n)-1
         ...
         if comienzo==1
            ...
            if mueBusq+T(n)<=IniSeg+tam_seg(n)-1
               [P(k),pos]=min(s(mueBusq:mueBusq+T(n)));
               i_P(k)=pos+(mueBusq-1);
               % The peak that is found serves as a reference to search the
               % next one
               mueBusq=i_P(k);
               k=k+1;
               comienzo=0;
         ...
         % If the first peak has been found (comienzo=0)   
            ...
            [P(k),pos]=min(s(mueIniBusq:mueFinBusq));
...
% Signal containing the negative peaks
Ns=length(s);
picos=zeros(1,Ns);
if ~isempty(P)
   picos(i_P)=P;
